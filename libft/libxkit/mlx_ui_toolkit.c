/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   mlx_ui_toolkit.c                                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mmalie <mmalie@student.42nice.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/10 23:09:06 by mmalie            #+#    #+#             */
/*   Updated: 2025/01/13 12:16:51 by mmalie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "./libxkit.h"

/* XXX Drawing UI elements */
//void	draw_health_bar(t_env *env, t_pos pos, int width, int height, //
		//float health_percent);
//void	draw_score(t_env *env, t_pos pos, int score, int color);
//void	draw_button(t_env *env, t_pos pos, int width, int height, //
		//char *text, int color);
//int	is_button_clicked(int button_x, int button_y, int width, int height, 
		//int mouse_x, int mouse_y);
