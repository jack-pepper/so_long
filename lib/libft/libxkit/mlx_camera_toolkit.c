...
/*                                                    +:+ +:+         +:+     */
/*   By: mmalie <mmaenter_cam_on_hero(state->cam, state->hero->pos, state->env);lie@student.42nice.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/15 18:37:27 by mmalie            #+#    #+#             */
/*   Updated: 2025/02/02 22:12:47 by mmalie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_cam(t_state *state)
{
	...
	state->cam->pos.y = 0;
	state->cam->pos.x = 0;
	state->cam->max.y = (state->map->tm_rows * RES_PIX) - state->env->canvas->height;
	state->cam->max.x = (state->map->tm_cols * RES_PIX) - state->env->canvas->width;
